...
void EncodeP1( inout uint4 block, inout float blockMSLE, float3 texels[ 16 ] )
{
    ...
    for ( j = 1; j < 16; ++j )
    {
        blockMin = min( blockMin, texels[ j ] );
        blockMax = max( blockMax, texels[ j ] );
    }
    ...
    for ( j = 0; j < 16; ++j )
    {
        refinedBlockMin = min( refinedBlockMin, texels[ j ] == blockMin ? refinedBlockMin : texels[ j ] );
        refinedBlockMax = max( refinedBlockMax, texels[ j ] == blockMax ? refinedBlockMax : texels[ j ] );
    }
    ...
    for ( uint k = 0; k < 16; ++k )
    {
        float texelPos = f32tof16( dot( texels[ k ], blockDir ) );
        indices[ k ] = ComputeIndex4( texelPos, endPoint0Pos, endPoint1Pos );
    }
    ...
    for ( uint l = 0; l < 16; ++l )
    {
        float weight = floor( ( indices[ l ] * 64.0f ) / 15.0f + 0.5f );
        float3 texelUnc = FinishUnquantize( endpoint0Unq, endpoint1Unq, weight );

        msle += CalcMSLE( texels[ l ], texelUnc );
    }
...
void EncodeP2Pattern( inout uint4 block, inout float blockMSLE, int pattern, float3 texels[ 16 ] )
{
    ...
    for ( uint m = 0; m < 16; ++m )
    {
        float p0TexelPos = f32tof16( dot( texels[ m ], p0BlockDir ) );
        float p1TexelPos = f32tof16( dot( texels[ m ], p1BlockDir ) );
        ...
        uint paletteID = Pattern( pattern, m );
        indices[ m ] = paletteID == 0 ? p0Index : p1Index;
    }
    ...
    for ( uint n = 0; n < 16; ++n )
    {
        uint paletteID = Pattern( pattern, n );
        ...
        float weight = floor( ( indices[ n ] * 64.0f ) / 7.0f + 0.5f );
        ...
        msle76 += CalcMSLE( texels[ n ], texelUnc76 );
        msle95 += CalcMSLE( texels[ n ], texelUnc95 );
    }

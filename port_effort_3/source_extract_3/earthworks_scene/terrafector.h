...
public:
	...
	void rebuildAll();
    Texture::SharedPtr getDisplayTexture();     // STRIP-REVIEW: kept — consumed by Earthworks_4::onRenderOverlay (non-ImGui blit path); dispTexIndex is only set >=0 by stripped GUI, so effectively editor-only

	std::vector<terrafectorEditorMaterial>	materialVector;
	int selectedMaterial = -1;      // STRIP-REVIEW: kept — read/written by terrain.cpp stamp code (currentStamp_to_Bezier etc.), not only GUI
	std::vector<Texture::SharedPtr>			textureVector;
	int dispTexIndex = -1;          // STRIP-REVIEW: kept — see getDisplayTexture
...
public:
    ...
    Texture::SharedPtr		thumbnail = nullptr;    // STRIP-REVIEW: kept — written by materialCache::find_insert_material (load path); only reader was stripped GUI

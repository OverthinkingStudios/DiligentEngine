...
bool testTile(uint t, inout float hitDistance)
{
    bool found = false;
    float4 sphere;
...
void main(uint3 coord : SV_DispatchThreadId)
{
    ...
    if (groundcover_feedback[0].tum_idx > 0 && groundcover_feedback[0].tum_idx < 1001)
    {
        quick = testTile(groundcover_feedback[0].tum_idx, hitDistance);
    }
    ...
    if (!quick)
    {
        ...
        {
            if ((tiles[t].flags & (main_CENTER)))		// it is on the bottom level		//??? add a a multiply in somehwrere
            {
                testTile(t, hitDistance);
            }
        }
    }

...
// A missing material or texture file has to be LOUD: quietly carrying on bakes
// a flat stamp into the terrain, which is near impossible to trace back later.
bool tfRequireFile(const std::filesystem::path& path, const char* what)
{
    if (std::filesystem::exists(path))
        return true;
    spdlog::error("terrafector: required {} not found - '{}'", what, path.string());
#if defined(_WIN32)
...
FILE* terrafectorSystem::_logfile = nullptr;    // opened by terrain.cpp onLoad
std::chrono::time_point<std::chrono::high_resolution_clock>  terrafectorSystem::logStartTime = std::chrono::high_resolution_clock::now();
uint logTab;
...
void tileTriangleBlock::insertTriangle(const uint _material, const float3 pos[3], const float2 uv[3])
{
    for (int i = 0; i < 3; i++)
    {
        ...
        tempIndexBuffer.push_back((uint)verts.size());
...
void tileTriangleBlock::insertTriangle(const uint _material, const uint _F[3], const aiMesh* _mesh, bool _yup)
{
    for (int i = 0; i < 3; i++)
    {
        ...
        tempIndexBuffer.push_back((uint)remapping[_F[i]]);
    }
}
...
void lodTriangleMesh_LoadCombiner::addMesh(std::string _path, lodTriangleMesh& _mesh, bool _remapMat)
{
    ...
    {
        uint startIndex = (uint)tiles[i].verts.size();
        uint startBuffer = (uint)tiles[i].tempIndexBuffer.size();
...
void lodTriangleMesh_LoadCombiner::loadToGPU(std::string _path, bool _log)
{
    ...
    for (auto& tile : tiles)
    {
        tfTile.numVerts = (uint)tile.verts.size();
        tfTile.numTriangles = (uint)tile.tempIndexBuffer.size();          // this is actualy indicis
        tfTile.numBlocks = (uint)ceil((float)tfTile.numTriangles / (128.0f * 3.0f));
        mostTri = __max(mostTri, (int)tfTile.numTriangles);



        // buffer indicis
        tile.tempIndexBuffer.resize((size_t)tfTile.numBlocks * 128 * 3);
        ...
        {
            tfTile.vertex = ew::Buffer::createStructured(
                sizeof(triVertex),
                tfTile.numVerts,
            ...
            tfTile.index = ew::Buffer::createStructured(
                sizeof(unsigned int),
            ...
            vertexData += (int)(sizeof(triVertex) * tfTile.numVerts);
            indexData += (int)(sizeof(unsigned int) * tfTile.numBlocks * 128 * 3);
        }
        else
        {
            // tfTile is reused across the whole loop, so empty slots have to
            // clear these or an empty grid cell keeps the previous tile's GPU
            // buffers alive.
            tfTile.vertex = nullptr;
            tfTile.index = nullptr;
        }
        ...
        if (file)
        {
            ...
            for (uint i = 0; i < tiles.size(); i++)
            {
                ...
                if (gpuTiles[i].numBlocks > 0)
                {
                    ...
                    fwrite(tiles[i].tempIndexBuffer.data(), sizeof(uint), (size_t)gpuTiles[i].numBlocks * 128 * 3, file);
                }
...
uint materialCache::find_insert_material(const std::string _path, const std::string _name)
{
    ...
    logTab--;
    // A mesh whose material cannot be found falls back to material 0, so say so
    // loudly rather than silently stamping the wrong material.
    spdlog::error("terrafector: material '{}' not found under '{}' (falls back to material 0)",
                  _name, terrafectorEditorMaterial::rootFolder + "/terrafectorMaterials");
...
uint materialCache::find_insert_material(const std::filesystem::path _path)
{
    ...
    for (uint i = 0; i < materialVector.size(); i++)
    {
        if (materialVector[i].fullPath.compare(_path) == 0)
        {
            ...
            if (!materialVector[i].thumbnail) {
                // Gated on existence: thumbnails are an editor-only asset, and
                // ew::createFromFile logs missing files loudly - it would spam
                // for every material without a .jpg.
                if (std::filesystem::exists(_path.string() + ".jpg"))
                    materialVector[i].thumbnail = ew::Texture::createFromFile(_path.string() + ".jpg", false, true);
            }
    ...
    if (std::filesystem::exists(_path.string() + ".jpg"))   // optional thumbnail, see above
        material.thumbnail = ew::Texture::createFromFile(_path.string() + ".jpg", false, true);
...
{
    logTab++;
    for (uint i = 0; i < texturePathVector.size(); i++)
    {
    ...
    {
        // On-the-fly DDS compression only happens when the hardcoded
        // Compressonator install is present (pipeline-tool behaviour).
        // Without it: LOUD error, and createFromFile below falls back to a 1x1
        // TRANSPARENT dummy (alpha 0, so bake consumers skip it).
        if (std::filesystem::exists("F:\\terrains\\_resources\\Compressonator\\CompressonatorCLI.exe"))
        {
        ...
        {
            tfRequireFile(ddsFilename, "terrafector texture (.earthworks.dds - precompress or install CompressonatorCLI)");
        }
    }
    ew::Texture::SharedPtr tex = ew::Texture::createFromFile(ddsFilename, true, isSRGB);
    if (tex)
    {
        // ew::Texture stores neither source path nor name, so materialCache
        // mirrors both index-aligned with textureVector (see terrafector.h).
        textureVector.emplace_back(tex);
        texturePathVector.emplace_back(_path);
        textureNameVector.emplace_back(_path.filename().string());
        ...
        for (uint i = 0; i < logTab; i++)   fprintf(terrafectorSystem::_logfile, "  ");
        fprintf(terrafectorSystem::_logfile, "%s\n", textureNameVector.back().c_str());
        fflush(terrafectorSystem::_logfile);

        logTab--;
        return (int)(textureVector.size() - 1);
    }
    else
    {
        for (uint i = 0; i < logTab; i++)   fprintf(terrafectorSystem::_logfile, "  ");
...
ew::Texture::SharedPtr materialCache::getDisplayTexture()
{
    if (dispTexIndex >= 0) {
...
void materialCache::setTextures(ew::pixelShader& _shader)
{
    // The layer takes the used prefix and dummy-pads the rest of the declared
    // gmyTextures_T[4096] slots; unset descriptor-array elements are UB on
    // Vulkan. terrafector.h sketches the bindless alternative.
    _shader.setTextureArray("gmyTextures_T", textureVector);
}
...
void terrafectorElement::loadPath(std::string _path)
{
    if (std::filesystem::exists(_path))
    {
        for (const auto& entry : std::filesystem::directory_iterator(_path))
        {
            ...
            {
                // The Steg 001_forest.fbx data is stale - its
                // forests.terrafectorMaterial is a pre-v101 file - and is due
                // to be replaced by the plant-data update. Remove this
                // exemption once that data lands.
                if (entry.path().filename().string().find("001_forest") != std::string::npos)
                {
                    spdlog::warn("terrafector: skipping '{}' (stale Steg forest data - awaiting plant-data update)", newPath);
                    continue;
                }
                ...
                if (ext.find("fbx") != std::string::npos ||
                    ...
                    find_insert(entry.path().filename().string(), tf_fbx, newPath);
                }
            }
        }
    }
...
void terrafectorEditorMaterial::import(std::filesystem::path  _path, bool _replacePath) {
    ...
    if (is.fail()) {
        ...
        fullPath = _path;
        // A failed import leaves _constData at its defaults (useElevation = 0,
        // a harmless no-op in the bake) but every layer referencing this
        // material then stamps the wrong look - be LOUD.
        tfRequireFile(_path, "terrafector material (.terrafectorMaterial)");
    }
    else
    {
        // .terrafectorMaterial files embed no version, so the read happens at a
        // HARDCODED TFMATERIAL_VERSION_LOAD. A file written before the v101
        // isStamp fields makes cereal THROW "NVP not found" - and the shipped
        // gameroot is FULL of pre-v101 files (asphalt, road markings, ...), so
        // the throw has to be caught. The retry as v100 is lossless: the
        // version gate only guards isStamp/stampWidth/stampHeight, every other
        // field reads identically. A file that fails BOTH reads logs loudly and
        // stays a provable no-op (useElevation = 0).
        bool loaded = false;
        try
        {
        ...
        {
            is.close();
            std::ifstream is100(_path);
            try
            {
                cereal::JSONInputArchive archive(is100);
                serialize(archive, 100);
                loaded = true;
                spdlog::info("terrafector: material '{}' loaded as v100 (pre-isStamp file)", _path.filename().string());
            }
            catch (const std::exception& ex)
            {
                displayName = "failed to load";
                fullPath = _path;
                spdlog::error("terrafector: material '{}' failed to parse as v{} AND v100 ({}) - material is a no-op in the bake",
                              _path.string(), TFMATERIAL_VERSION_LOAD, ex.what());
            }
        }
...
void terrafectorEditorMaterial::rebuildConstantbufferData()
{
    ...
    // An ABSOLUTE-elevation material whose elevation textures failed to load
    // writes `alpha * (YOffset + 0)` as REPLACEMENT height - the exact
    // signature of the y=0 bug, terrain silently flattened under the material's
    // footprint. (-1 texture indices become 0xffffffff and sample the dummy = 0.)
    if (_constData.useElevation && _constData.useAbsoluteElevation > 0.5f && !_constData.useVertexY)
    {
        const bool baseMissing = texturePaths[baseElevation].size() && textureIndex[baseElevation] < 0;
        const bool detailMissing = texturePaths[detailElevation].size() && textureIndex[detailElevation] < 0;
        if (baseMissing || detailMissing)
            spdlog::error("terrafector: material '{}' uses ABSOLUTE elevation but its elevation texture(s) failed to load - would flatten terrain to YOffset={} under its footprint",
                          displayName, _constData.YOffset);
    }
}
...
void terrafectorSystem::exportMaterialBinary(std::string _path, std::string _evoRoot)
{
    ...
    if (file)
    {
        ...
        for (uint i = 0; i < textureSize; i++) {
            std::string path = terrafectorEditorMaterial::static_materials.texturePathVector[i].string();
            path = path.substr(13, path.size());    // brittle EVO-export magic offset
            path = path.substr(0, path.rfind("."));
            path += ".texture";
            memset(name, 0, 512);
            snprintf(name, sizeof(name), "%s", path.c_str());

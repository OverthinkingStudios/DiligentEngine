...
#include "terrain.h"   // shaderLightBuffer (via vegetationBuilder.h) - matches the ecotope.cpp include pattern
...
// PORT-REVIEW (step 6 salvage): logic identical to legacy/core/buildings.cpp
// (which restored the commented-out "rappersville" blocks in the original
// terrain.cpp / Earthworks_4.cpp). Only the GPU surface changed: Falcor compat
// Vars()/State() -> ew:: named binds, rmcv matrices -> transposed glm
// (ewCamera.h convention - callers already hold the transposed matrices).

// float2/float3/float4 are the global glm aliases from terrain.h's
// HLSL-shared block (identical to the ew:: aliases).
...
void buildingsRenderer::load(const std::string& _basePath)
{
    ...
    vertexData = ew::Buffer::createStructured(sizeof(vertex), static_cast<uint32_t>(numVerts),
                                              Diligent::BIND_SHADER_RESOURCE, verts.data(), "buildings vertexData");
    cpuVerts = std::move(verts);        // kept for overlayShadowHeights()

    shader.load("hlsl/terrain/render_Buildings_Far.hlsl", "vsMain", "psMain", ew::Topology::TriangleList);
    shader.setBuffer("vertexBuffer", vertexData);

    // The original never bound samplers here (Falcor supplied defaults).
    // shadow() uses gSmpLinear, sunLight()/atmosphere use gSmpLinearClamp -
    // linear clamp suits both.
    {
        ...
        shader.setSampler("gSmpLinear", sampler_Clamp);
        shader.setSampler("gSmpLinearClamp", sampler_Clamp);
    }
...
void buildingsRenderer::buildChunks(std::vector<vertex>& _verts)
{
    ...
    std::vector<uint32_t> cellCount(static_cast<size_t>(cellsX) * cellsY, 0);
    for (size_t t = 0; t < numTris; t++)
        cellCount[cellOf(&_verts[t * 3])]++;

    std::vector<uint32_t> cellStart(static_cast<size_t>(cellsX) * cellsY);   // in triangles
    uint32_t running = 0;
...
void buildingsRenderer::updateShaderConstants(ew::Texture::SharedPtr _terrainShadow, const shaderLightBuffer& _buffer)
{
    if (!loaded()) return;

    shader.setTexture("terrainShadow", _terrainShadow);

    shader.setVariable("LightsCB", "sunDirection", _buffer.sunDirection);
    shader.setVariable("LightsCB", "sunRightVector", _buffer.sunRightVector);
    shader.setVariable("LightsCB", "sunUpVector", _buffer.sunUpVector);
    shader.setVariable("LightsCB", "screenSize", _buffer.screenSize);
    shader.setVariable("LightsCB", "fog_far_Start", _buffer.fog_far_Start);
    shader.setVariable("LightsCB", "fog_far_log_F", _buffer.fog_far_log_F);
    shader.setVariable("LightsCB", "fog_far_one_over_k", _buffer.fog_far_one_over_k);
}


void buildingsRenderer::setAtmosphere(ew::Texture::SharedPtr _inscatter, ew::Texture::SharedPtr _outscatter, ew::Texture::SharedPtr _sunlight)
{
    if (!loaded()) return;

    shader.setTexture("gAtmosphereInscatter", _inscatter);
    shader.setTexture("gAtmosphereOutscatter", _outscatter);
    shader.setTexture("SunInAtmosphere", _sunlight);
}


void buildingsRenderer::render(ew::GpuContext* _renderContext, const ew::Fbo::SharedPtr& _fbo,
                               const glm::mat4& _view, const glm::mat4& _viewproj, const float3& _eye,
                               const std::vector<float4>& _visibleTileRects)
{
    if (!loaded() || !ew::gDebug.toggles.buildings) return;

    // Default depth (on, write, Less) and cull-back with the layer's forced
    // CCW front - the same effective state the legacy integration drew with.
    shader.setFbo(_fbo);
    shader.setVariable("PerFrameCB", "view", _view);
    shader.setVariable("PerFrameCB", "viewproj", _viewproj);
    shader.setVariable("PerFrameCB", "eye", _eye);
    ...
    auto flush = [&]()
    {
        ...
        shader.setVariable("PerFrameCB", "firstVertex", runStart);
